...
uintptr_t find_interval_1(size_t size) {
    size_t align = round_power2(size);
    // assert(!(align & (align - 1))); //假设传入的begin已经对齐到2^i了
    uintptr_t left_bound = L, rtn = -1;
...
void find_and_free_1(uintptr_t l) {
    spin_lock(&fast_lock);
    Interval *itr = head;
    while (itr->next != NULL) {
        // printf("dead\n");
...
{
    if (len <= LIMIT) {
        return (void*)find_interval_1(size);
    }
    return (void *)find_interval(size);
}

void kfree(void *ptr) {
  if (len <= LIMIT) {
    find_and_free_1((void*)ptr);
    return;
  }
  find_and_free((void *)ptr);
}
