...
int getFreeList(int cpu_no) {
  ...
  for (int i = 0; i < BRANCH; i++)
  {
    assert(pages[i].busyByes <= HEAPSIZE);
...
void* kalloc(size_t size, int cpu_no) {
  ...
  while (itr->next != head[k])
  {
    ...
    if (valid_start != NULL)
    {
      pages[k].busyByes += size;
      spin_unlock(&locks[k]);
      return valid_start;
    }
    itr = itr->next;
  }
  // printf("old:cpu_no = %d, k = %d, busyBytes=%ld\n", cpu_no, k, pages[freelist[cpu_no]].busyByes);
  spin_unlock(&locks[k]);
  k = getFreeList(cpu_no);
  spin_lock(&locks[k]);
  itr = head[k];
  assert(head[k]->next != head[k]);
  while (itr->next != head[k])
  {
    void *valid_start = alloc_split(itr->next, size, seg[k] - (HEADSIZE - WORDSIZE));
    if (valid_start != NULL)
  ...
  // printf("new:cpu_no = %d, k = %d, busyBytes=%ld\n", cpu_no, k, pages[freelist[cpu_no]].busyByes);
  spin_unlock(&locks[k]);
  return NULL;
}
